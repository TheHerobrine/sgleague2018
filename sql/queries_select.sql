...
BEGIN
  ...
  LEFT JOIN T_PLATFORM_USER ON PU_ID_P=P_UID AND PU_ID_SU=id_user LEFT JOIN T_GAME ON G_ID_P=P_UID LEFT JOIN T_GAME_USER ON GU_ID_SU=id_user ORDER BY P_UID;
END|
...
CREATE PROCEDURE SEARCH_SCHOOL( IN school VARCHAR(512) )
BEGIN
...
BEGIN
  SELECT SU_LOGIN, SU_UID, PU_PSEUDO, GU_PSEUDO, GU_RANK, S_NAME, SU_FIRST_NAME, SU_LAST_NAME FROM T_SGL_USER LEFT JOIN T_SCHOOL ON SU_ID_S=S_UID JOIN T_GAME ON id_game=G_UID
